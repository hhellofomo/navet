    ...
    if (stored) {
      try {
        ...
        const allDeviceIds = new Set(
          Object.values(devices).flat().map(d => d.id)
        );
        const isValid = Object.values(parsed).every((orderArray: any) =>
          Array.isArray(orderArray) && orderArray.every(id => allDeviceIds.has(id))
        );
        if (isValid) {
          return parsed;
        }
