...
export const WifiCard = memo(function WifiCard({ networkName, speed, uploadSpeed, downloadSpeed, size, onSizeChange, isEditMode }: WifiCardProps) {
  const cardId = 'wifi-1';
  ...
  // Theme-aware colors
  const cardGradient = theme === 'light' ? 'from-white to-green-50/80' : 'from-green-900/90 to-green-950/95';
      ...
      <div className="relative h-full flex flex-col">
        <div className={`flex items-start justify-between ${isSmall ? 'mb-1' : 'mb-2'}`}>
          <div className="min-w-0 flex-1">
            <h3 className={`font-semibold ${textPrimary} truncate ${isSmall ? 'text-xs' : 'text-sm'}`}>Wi-Fi</h3>
        ...
        {isSmall ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className={`text-2xl font-bold ${textPrimary} leading-none`}>{speed}<span className="text-xs ml-1">Mbps</span></div>
          </div>
        ) : isMedium ? (
          // Medium: Speed with compact stats
          <>
            <div className="flex-1 flex flex-col justify-center">
              <div className={`text-3xl font-bold ${textPrimary} mb-1`}>{speed} <span className="text-base">Mbps</span></div>
