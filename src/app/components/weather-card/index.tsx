...
import { MapPin, Clock, Sunrise, Sunset, Info } from 'lucide-react';
import { CardSizeSelector, type CardSize } from '../card-size-selector';
import { CardWrapper } from '../ui/card-wrapper';
import { CaptionValue } from '../ui/caption-value';
import { WeatherIcon, type WeatherCondition } from './weather-icon';
...
export const WeatherCard = memo(function WeatherCard({ 
  ...
  const glowOverlay = theme === 'light' ? 'from-blue-50/30 via-transparent to-cyan-50/30' : 'from-blue-500/5 via-transparent to-cyan-500/5';
  const dashedBorder = theme === 'light' ? 'border-gray-300' : 'border-slate-600';
  
  return (
    <CardWrapper
      className={`${cardBg} p-5`}
    >
      {isEditMode && (
        <CardSizeSelector 
          currentSize={size} 
      ...
      <div className="relative z-[2] h-full flex flex-col">
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="min-w-0 flex-1">
            <h3 className={`font-semibold ${textPrimary} truncate ${isSmall ? 'text-xs' : 'text-sm'}`}>{location}</h3>
            {!isSmall && (
              <p className={`text-xs ${textSecondary}`}>{dateTime}</p>
            )}
          </div>
          <div className={`${isSmall ? 'w-8 h-8' : 'w-10 h-10'} rounded-full flex items-center justify-center flex-shrink-0 ${iconBg}`}>
        ...
        {isSmall ? (
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <div className={`text-xs ${textSecondary} mt-0.5`}>H:{highTemp}° L:{lowTemp}°</div>
          </div>
        ) : (
          <div className="mt-auto">
            ...
            <div className="flex items-end justify-between mb-3">
              ...
              <div className="flex-shrink-0">
                <div className={`text-3xl font-bold ${textPrimary} leading-none mb-1`}>{temperature}°C</div>
                <div className={`text-xs ${textSecondary} mb-0.5`}>H:{highTemp}° L:{lowTemp}°</div>
                {rainForecast && (
              ...
              <div className="flex justify-between gap-3">
                {forecast.map((day, index) => (
                  ...
                  >
                    ...
                    <div className={`text-xs ${theme === 'light' ? 'text-gray-400' : 'text-gray-500'}`}>{day.low}°</div>
                  </div>
                ))}
              </div>
            )}
