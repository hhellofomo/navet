...
export const PersonCard = memo(function PersonCard({ name, location, state, size, onSizeChange, isEditMode }: PersonCardProps) {
  ...
  const textColor = theme === 'light' ? (state === 'home' ? 'text-gray-900' : 'text-gray-500') : (state === 'home' ? 'text-white' : 'text-gray-500');
  const subTextColor = theme === 'light' ? (state === 'home' ? 'text-gray-600' : 'text-gray-400') : (state === 'home' ? 'text-gray-400' : 'text-gray-600');
  ...
  const statusLabelColor = theme === 'light' 
    ? (state === 'home' ? 'text-gray-700' : 'text-gray-500') 
    : (state === 'home' ? 'text-gray-400' : 'text-gray-400');
      ...
      <div className="relative h-full flex flex-col">
        <div className="flex items-start justify-between mb-2">
          <div className="min-w-0 flex-1">
            <h3 className={`font-semibold truncate ${isSmall ? 'text-xs' : 'text-sm'} ${textColor}`}>{name}</h3>
            {!isSmall && <p className={`text-xs ${subTextColor}`}>{location}</p>}
          </div>
        </div>

        <div className="flex-1 flex flex-col items-center justify-center">
          <div className={`${isSmall ? 'w-10 h-10' : 'w-14 h-14'} rounded-full flex items-center justify-center mb-2 ${
            state === 'home' 
              ? 'bg-gradient-to-br from-blue-400 to-blue-600 shadow-lg shadow-blue-500/50' 
              : avatarAwayBg
          }`}>
            <User className={`${isSmall ? 'w-5 h-5' : 'w-7 h-7'} ${state === 'home' ? 'text-white' : 'text-gray-600'}`} />
          </div>
