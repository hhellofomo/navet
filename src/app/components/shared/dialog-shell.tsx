...
/**
 * Returns the standard centering + style class string for a settings dialog content panel.
 * Use this instead of hand-assembling the same `fixed top-1/2 left-1/2 z-50 …` string
 * in every settings dialog.
 */
export function settingsDialogContentClass(
  ...
  options?: {
    ...
    height?: 'tall' | 'capped';
    /** Adds `overflow-hidden`. Defaults to `true` when `height` is set. */
    overflow?: boolean;
    /** Adds `p-6` padding to the content panel. @default false */
    padding?: boolean;
    /** Adds `animate-in fade-in zoom-in duration-200`. @default false */
    animate?: boolean;
  }
): string {
  const {
