...
export const ColorTemperaturePresets = memo(function ColorTemperaturePresets({
  ...
  const scaleSelected = size === 'small' ? 'scale-110' : size === 'medium' ? 'scale-110' : 'scale-[1.15]';
  const glowSize = size === 'small' 
      ...
      {options.map((option) => (
        <button
          ...
          style={{ 
            ...
            boxShadow: currentTemp === option.value && !selectedColor && isOn 
