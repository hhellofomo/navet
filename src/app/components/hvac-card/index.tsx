...
export const HVACCard = memo(function HVACCard({ 
  ...
  const getCardColors = () => {
    ...
    switch(mode) {
      case 'cool': return colors.hvac.cooling;
      case 'heat': return colors.hvac.heating;
      default: return colors.hvac.off;
    }
  };
  ...
  const lightOverlay = theme === 'light'
    ? isOn
      ? mode === 'cool' ? 'bg-cyan-50/45' : mode === 'heat' ? 'bg-orange-50/45' : 'bg-white/60'
      : 'bg-white/60'
    : undefined;
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`absolute inset-0 bg-gradient-to-br ${cardColors.glow} to-transparent transition-all duration-500`}></div>
        
        <div className="relative z-[2] h-full flex flex-col">
          {/* Header */}
          <div className={`flex items-start justify-between ${isSmall ? 'mb-1' : 'mb-2'}`}>
            <div className="min-w-0 flex-1">
              <h3 className={`font-semibold ${textColor} truncate ${isSmall ? 'text-xs' : 'text-sm'} transition-colors duration-500`}>{name}</h3>
            </div>
            <div className={`${isSmall ? 'w-8 h-8' : 'w-10 h-10'} rounded-full ${cardColors.iconBg} flex items-center justify-center flex-shrink-0 transition-all duration-500`}>
              <Wind className={`${isSmall ? 'w-4 h-4' : 'w-5 h-5'} ${cardColors.accent} transition-colors duration-500`} />
            </div>
          </div>
          ...
          {isSmall ? (
            ...
            <div className="flex-1 flex flex-col justify-end gap-2">
              ...
              <div>
                <div className={`text-3xl font-bold ${textColor} leading-none transition-colors duration-500 mb-1`}>{targetTemp}°C</div>
                <div className={`text-xs ${secondaryTextColor}`}>Current temperature {currentTemp}°C</div>
              </div>
