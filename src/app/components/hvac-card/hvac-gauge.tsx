...
import { getHVACGaugeColor, getHVACGlowColor, getHVACTextShadow, getHVACBackgroundGlowColor } from '../../utils/hvac-styles';
...
export const HVACGauge = memo(function HVACGauge({ 
  ...
  const tempTextColor = theme === 'light' 
      ...
      <svg className="w-full h-full" viewBox="0 0 220 140">
        ...
        <defs>
          <linearGradient id={`gauge-gradient-${id}`} x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <stop offset="100%" stopColor={gaugeColors.secondary} stopOpacity={isOn ? "1" : "0.5"} />
          </linearGradient>
          <filter id={`glow-${id}`}>
