...
import albumArt from 'figma:asset/847d39d7e328a23edbec0f0c53ec4c57b6f1d6fb.png';
...
export const MediaCard = memo(function MediaCard({ title, artist, size, onSizeChange, isEditMode }: MediaCardProps) {
  ...
  return (
    <>
      <div className={`relative h-full backdrop-blur-xl rounded-3xl ${padding} border ${isLight ? 'border-gray-200/50 shadow-lg' : 'border-pink-700/20'} overflow-hidden`}>
        {isEditMode && (
          <CardSizeSelector 
            currentSize={size} 
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          />
          <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-b from-white/70 via-white/50 to-white/80' : 'bg-gradient-to-b from-black/60 via-black/40 to-black/80'}`}></div>
        </div>
        
        <div className={`absolute inset-0 bg-gradient-to-br ${isLight ? 'from-pink-200/20' : 'from-pink-500/10'} to-transparent`}></div>
        
        <div className="relative h-full flex flex-col">{isSmall ? (
            ...
            >
              ...
              <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-b from-white/30 via-transparent to-white/60 opacity-60 group-hover:opacity-40' : 'bg-gradient-to-b from-black/30 via-transparent to-black/60 opacity-60 group-hover:opacity-40'} transition-opacity`}></div>
              {/* Play indicator on hover */}
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <div className={`w-16 h-16 rounded-full ${isLight ? 'bg-white/40' : 'bg-black/40'} backdrop-blur-md flex items-center justify-center`}>
                  {isPlaying ? (
                    <Pause className={`w-8 h-8 ${isLight ? 'text-gray-800' : 'text-white'}`} fill={isLight ? '#1f2937' : 'white'} />
                  ) : (
                    <Play className={`w-8 h-8 ${isLight ? 'text-gray-800' : 'text-white'}`} fill={isLight ? '#1f2937' : 'white'} />
                  )}
                </div>
              </div>
            </div>
            ...
            <>
              <div className="flex-1 flex items-stretch gap-3 -m-5">
                ...
                <div className="relative w-40 flex-shrink-0 overflow-hidden">
                  ...
                  <div className={`absolute inset-y-0 right-0 w-12 ${isLight ? 'bg-gradient-to-l from-white/80 to-transparent' : 'bg-gradient-to-l from-black/80 to-transparent'}`}></div>
                </div>
                
                <div className="flex-1 flex flex-col justify-center gap-3 min-w-0 pr-5 py-5">
                  <div>
                    <div className={`font-bold truncate text-sm ${isLight ? 'text-gray-900' : 'text-white'}`}>{title}</div>
                    <div className={`text-xs truncate ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>{artist}</div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className={`flex-1 relative h-1 ${isLight ? 'bg-gray-900/15' : 'bg-white/20'} rounded-full overflow-hidden`}>
                      <div 
                    ...
                    <span className={`text-[10px] w-6 text-right ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>{isMuted ? 0 : volume}</span>
                  </div>
                </div>
              </div>
            </>
            ...
            <>
              <div className="flex-1 flex flex-col items-center justify-center gap-4">
                ...
                <div className="text-center w-full">
                  <div className={`font-bold truncate text-lg ${isLight ? 'text-gray-900' : 'text-white'}`}>{title}</div>
                  <div className={`text-sm truncate ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>{artist}</div>
                </div>

                <div className="flex items-center justify-center gap-4">
                  <button className={`w-10 h-10 rounded-full ${isLight ? 'bg-gray-900/10 hover:bg-gray-900/20' : 'bg-white/10 hover:bg-white/20'} flex items-center justify-center transition-colors`}>
                ...
                <div className="w-full flex items-center gap-3 px-2">
                  ...
                  <span className={`text-xs w-8 text-right ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>{isMuted ? 0 : volume}</span>
                </div>
              </div>
            </>
          )}
