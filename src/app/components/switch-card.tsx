...
import { Power, Zap, Plug, Calendar } from 'lucide-react';
...
export const SwitchCard = memo(function SwitchCard({ name, room, initialState = false, power, voltage, energy }: SwitchCardProps) {
  ...
  const textColor = theme === 'light' ? (isOn ? 'text-gray-900' : 'text-gray-500') : (isOn ? 'text-white' : 'text-gray-500');
      ...
      <div className="relative h-full flex flex-col">
        <div className="flex items-start justify-between mb-2">
          <div className="min-w-0 flex-1">
            <h3 className={`font-semibold text-xs ${textColor} transition-colors duration-500 truncate text-left`}>{name}</h3>
          </div>
