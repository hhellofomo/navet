...
export const RoomNav = memo(function RoomNav({ activeRoom, onRoomChange, isEditMode, onToggleEditMode, onAddCard }: RoomNavProps) {
  ...
  const inactiveBg = theme === 'light' ? 'bg-gray-100' : theme === 'contrast' ? 'bg-black/50' : 'bg-white/5';
  const hoverBg = theme === 'light' ? 'hover:bg-gray-200' : theme === 'contrast' ? 'hover:bg-white/20' : 'hover:bg-white/10';
  const border = theme === 'light' ? 'border-gray-200' : theme === 'contrast' ? 'border-white/20' : 'border-white/5';

  return (
    <div className={`flex items-center gap-2 pb-4 md:pb-6 border-b ${border} mb-6 md:mb-8 overflow-x-auto scrollbar-hide`}>
      {rooms.map((room) => (
        <button
          key={room}
