...
import { Blinds, ChevronUp, ChevronDown, Square, DoorOpen, Settings, Fence, Home, SunDim, ShieldCheck } from 'lucide-react';
...
type DeviceClass = 'blind' | 'shade' | 'curtain' | 'garage' | 'gate' | 'awning' | 'shutter' | 'door';
...
const deviceClassConfig: Record<DeviceClass, { label: string; icon: any }> = {
  ...
  shutter: { label: 'Window Shutters', icon: ShieldCheck },
  door: { label: 'Sliding Doors', icon: DoorOpen },
};
...
export const CoverCard = memo(function CoverCard({ 
  ...
  const [coverState, setCoverState] = useState<CoverState>(initialPosition === 100 ? 'open' : initialPosition === 0 ? 'closed' : 'open');
  ...
  const settingsBtnClass = theme === 'light'
  ...
  return (
    <div className={`relative h-full bg-gradient-to-br ${cardColors.gradient} backdrop-blur-xl rounded-3xl ${padding} border ${cardColors.border} overflow-hidden ${theme === 'light' ? 'shadow-lg' : ''}`}>
      ...
      <div className="relative h-full flex flex-col">
        <div className={`flex items-start justify-between ${isSmall ? 'mb-1' : 'mb-2'}`}>
          <div className="min-w-0 flex-1">
            <h3 className={`font-semibold ${textColor} truncate ${isSmall ? 'text-xs' : 'text-sm'}`}>{name}</h3>
            {!isSmall && (
          ...
          <div className={`${isSmall ? 'w-8 h-8' : 'w-10 h-10'} rounded-full ${cardColors.iconBg} flex items-center justify-center flex-shrink-0`}>
      ...
      <Dialog.Root open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gradient-to-br from-gray-900/95 to-gray-950/95 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-6 w-[90vw] max-w-md z-50 shadow-2xl animate-in fade-in zoom-in duration-200">
            <Dialog.Title className="text-xl font-semibold text-white mb-2">Device Type</Dialog.Title>
            ...
            <div className="flex gap-3">
              <Dialog.Close asChild>
                <button className="flex-1 py-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-white text-sm font-medium transition-colors">
                  Cancel
                </button>
              </Dialog.Close>
              <Dialog.Close asChild>
                <button className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 rounded-xl text-white text-sm font-medium transition-colors">
                  Done
                </button>
              </Dialog.Close>
            </div>
