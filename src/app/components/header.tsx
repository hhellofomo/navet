...
export const Header = memo(function Header() {
  ...
  const { searchQuery, setSearchQuery, setFilteredDeviceIds, clearSearch, isSearchActive } = useSearch();
  ...
  useEffect(() => {
    ...
    const searchInDevices = (deviceArray: any[] | undefined, searchFields: string[]) => {
  ...
  const textPrimary = theme === 'light' ? 'text-gray-900' : theme === 'contrast' ? 'text-white' : 'text-white';
  const textSecondary = theme === 'light' ? 'text-gray-600' : theme === 'contrast' ? 'text-gray-300' : 'text-gray-400';
  const inputBg = theme === 'light' ? 'bg-gray-100' : theme === 'contrast' ? 'bg-black/50' : 'bg-white/5';
  const inputBorder = theme === 'light' ? 'border-gray-200' : theme === 'contrast' ? 'border-white/20' : 'border-white/5';
  const inputFocusBorder = theme === 'light' ? 'focus:border-gray-300' : theme === 'contrast' ? 'focus:border-white/40' : 'focus:border-white/10';
  const placeholder = theme === 'light' ? 'placeholder-gray-400' : theme === 'contrast' ? 'placeholder-gray-400' : 'placeholder-gray-500';
  const hoverBg = theme === 'light' ? 'hover:bg-gray-100' : theme === 'contrast' ? 'hover:bg-white/10' : 'hover:bg-white/5';
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-8 gap-4">
      <div className="flex items-center gap-4 md:gap-6 flex-1">
        <div>
          ...
          <p className={`${textSecondary} text-xs md:text-sm`}>Take full control of your home instantly</p>
        </div>
      </div>
