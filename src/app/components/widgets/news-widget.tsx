...
export function NewsWidget({ size = 'large' }: NewsWidgetProps) {
  ...
  const bgColor = theme === 'light' ? 'bg-white/70' : theme === 'contrast' ? 'bg-black/50' : 'bg-white/10';
  ...
  const displayArticles = size === 'small' ? mockArticles.slice(0, 2) : size === 'medium' ? mockArticles.slice(0, 3) : mockArticles;
    ...
    <div className={`${bgColor} backdrop-blur-xl rounded-2xl p-4 border ${border} h-full flex flex-col`}>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div 
          ...
          style={{ backgroundColor: `${getColorValue(primaryColor)}20`, color: getColorValue(primaryColor) }}
        >
          ...
          <div key={article.id}>
            <div className="flex items-start gap-3 group cursor-pointer">
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-sm font-medium ${textPrimary} mb-1 group-hover:underline line-clamp-2`}>
                  {article.title}
                </p>
              ...
              <ExternalLink className={`w-4 h-4 ${textSecondary} opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0`} />
            </div>
