...
export const PowerCard = memo(function PowerCard({ percentage, usage, cost, size, onSizeChange, isEditMode }: PowerCardProps) {
  ...
  // Theme-aware colors
  const cardGradient = theme === 'light' ? 'from-white to-yellow-50/80' : 'from-yellow-900/90 to-yellow-950/95';
      ...
      <div className="relative h-full flex flex-col">
        <div className={`flex items-start justify-between ${isSmall ? 'mb-1' : 'mb-2'}`}>
          <div className="min-w-0 flex-1">
            <h3 className={`font-semibold ${textPrimary} truncate ${isSmall ? 'text-xs' : 'text-sm'}`}>Energy Usage</h3>
          </div>
            ...
            <div className="flex-1 flex flex-col justify-center gap-2">
              <div className="flex items-baseline gap-2">
                ...
                <div className={`flex items-center gap-1 px-2 py-0.5 ${savingBg} rounded-full`}>
                  ...
                  <span className={`text-[10px] ${savingText} font-medium`}>Saving ${savedAmount}</span>
                </div>
              </div>
