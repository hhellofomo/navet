...
import { Snowflake, Flame, Wind, Power } from 'lucide-react';
...
export const HVACSettingsDialog = memo(function HVACSettingsDialog({
  ...
  const getModeColor = () => {
    ...
    switch(mode) {
      case 'cool': return { from: 'blue-900', to: 'blue-950', border: 'blue-500/20' };
      case 'heat': return { from: 'red-900', to: 'red-950', border: 'red-500/20' };
      case 'fan': return { from: 'green-900', to: 'green-950', border: 'green-500/20' };
      default: return { from: 'gray-900', to: 'gray-950', border: 'gray-500/10' };
    }
  };
  ...
  const getModeButtonColorWithShadow = (buttonMode: string) => {
    ...
    if (mode === buttonMode && isOn) {
      switch(buttonMode) {
        case 'cool': return `${baseColor} shadow-lg shadow-blue-500/30`;
        case 'heat': return `${baseColor} shadow-lg shadow-red-500/30`;
        case 'fan': return `${baseColor} shadow-lg shadow-green-500/30`;
      }
    }
    return baseColor;
  };
                  ...
                  <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className={`text-4xl font-bold ${isOn ? 'text-white' : 'text-gray-500'} leading-none mt-1`}>
                          {targetTemp}°C
                        </div>
                      </div>
