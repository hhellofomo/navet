...
import { 
  ...
  Sun, SunMedium, SunDim, Sunrise, Sunset, SunMoon,
  Moon, MoonStar, Star, StarHalf,
  Zap, ZapOff, Sparkles, Sparkle,
  Flame, Flashlight, 
...
export const IconPicker = memo(function IconPicker({ selectedIcon, onIconChange, isLightOn }: IconPickerProps) {
  const [searchQuery, setSearchQuery] = useState('');

  // Filter icons based on search query
  const filteredIcons = searchQuery 
    ? lightIcons.filter(icon => 
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <span className={`text-sm font-medium transition-colors duration-500 ${isLightOn ? 'text-gray-300' : 'text-gray-500'}`}>
          Light Icon
        </span>
      </div>
      ...
      <div className="relative mb-4">
        <Search className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-500 ${isLightOn ? 'text-gray-400' : 'text-gray-600'}`} />
        <input
          type="text"
          value={searchQuery}
      ...
      {searchQuery && filteredIcons.length === 0 && (
        <div className={`text-center py-8 transition-colors duration-500 ${isLightOn ? 'text-gray-500' : 'text-gray-700'}`}>
