...
export const SensorCard = memo(function SensorCard({ name, room, value, unit, icon = 'gauge', size, onSizeChange, isEditMode }: SensorCardProps) {
  ...
  const IconComponent = icon === 'trend-up' ? TrendingUp : icon === 'trend-down' ? TrendingDown : Gauge;

  // Theme-aware colors
  const cardGradient = theme === 'light' ? 'from-white to-teal-50/80' : 'from-teal-900/90 to-teal-950/95';
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className={`${isSmall ? 'text-3xl' : 'text-4xl'} font-bold ${textPrimary}`}>
              {value}
              <span className={`${isSmall ? 'text-lg' : 'text-2xl'} ${accentColor} ml-1`}>{unit}</span>
            </div>
          </div>
        </div>
      </div>
