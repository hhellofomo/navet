...
export const LockCard = memo(function LockCard({ name, room, initialState = true }: LockCardProps) {
  ...
  return (
    <div className={`relative h-full bg-gradient-to-br ${cardColors.gradient} backdrop-blur-xl rounded-3xl p-4 border ${cardColors.border} overflow-hidden transition-all duration-500 ${theme === 'light' ? 'shadow-lg' : ''}`}>
      ...
      <div className="relative h-full flex flex-col">
        <div className="flex items-start justify-between mb-2">
          ...
          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-500 ${cardColors.iconBg}`}>
            {isLocked ? (
              <DoorClosed className={`w-4 h-4 transition-colors duration-500 ${cardColors.accent}`} />
            ) : (
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <div className="text-center mt-3">
            <div className={`text-xs ${isLocked ? (theme === 'light' ? 'text-green-700' : 'text-green-400') : (theme === 'light' ? 'text-red-700' : 'text-red-400')} transition-colors duration-500`}>
