	...
	return (
		<div className={`flex items-start justify-between ${marginBottom}`}>
			...
			<div
				...
				style={isOn ? { backgroundColor: theme === 'light' ? `${activeColor.text}66` : activeColor.bg } : undefined}
			>
				<IconComponent
					aria-hidden="true"
					...
					style={isOn ? { color: theme === 'light' ? activeColor.text : activeColor.text } : undefined}
				/>
			</div>
		</div>
	);
