...
import type { UpsDeviceOption, UpsMetricKind, UpsMetricOption } from './ups-widget-data';
...
function buildHomeyUpsDeviceOptions(
  ...
  return Array.from(sensorsBySourceDevice.entries())
    .flatMap(([sourceDeviceId, sourceSensors]) => {
      ...
      const sourceDevice = Object.values(devicesByCanonicalId).find(
        (device) =>
          device.providerId === 'homey' &&
          device.nativeId === sourceDeviceId &&
          device.kind !== 'sensor'
      );
...
export function useProviderUpsWidgetData(
  ...
  const homeAssistantData = useHomeAssistantUpsWidgetData(
    options,
  ...
  return useMemo(() => {
    ...
    const sensors = mapNavetDevicesToDeviceCollection(
      Object.values(devicesByCanonicalId).filter(
        (device): device is NavetDevice => device.providerId === 'homey'
      )
    ).sensors;

    return {
      devices: buildHomeyUpsDeviceOptions(sensors, devicesByCanonicalId),
      entities: toHomeyUpsEntityMap(sensors),
      formatOptions: homeAssistantData.formatOptions,
    };
  }, [currentProviderId, devicesByCanonicalId, homeAssistantData]);
}
